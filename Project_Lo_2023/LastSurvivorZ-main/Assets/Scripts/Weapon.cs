...
public class Weapon : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isEquipped && !pickupInstructionsText.activeInHierarchy && curAmmo <= 0)
        {
            ...
            pickupInstructionsText.GetComponent<TextMeshProUGUI>().text = "Нажмите R, чтобы перезагрузить";
        }
    ...
    public IEnumerator Reload()
    {
        ...
        if (!isReloading)
        {
            ...
            pickupInstructionsText.SetActive(false);
            pickupInstructionsText.GetComponent<TextMeshProUGUI>().text = "Нажмите E, чтобы взять пистолет";
            isReloading = false;
        }
    }
